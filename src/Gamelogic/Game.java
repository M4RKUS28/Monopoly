...
public class Game {
	...
	public void ende() {
		...
		for (int i = 0; i < pla.length; i++) {
			if (max < pla[i].getGeld() + pla[i].getGesammtscore()) {
				max = pla[i].getGeld() + pla[i].getGesammtscore();
				p1 = i;
			}
		}
		max = -999999;
		for (int i = 0; i < pla.length; i++) {
			if (max < pla[i].getGeld() + pla[i].getGesammtscore() && (i != p1)) {
				max = pla[i].getGeld() + pla[i].getGesammtscore();
				p2 = i;
			}
		}
		max = -999999;
		for (int i = 0; i < pla.length; i++) {
			if (max < pla[i].getGeld() + pla[i].getGesammtscore() && (i != p1 && i != p2)) {
				max = pla[i].getGeld() + pla[i].getGesammtscore();
				p3 = i;
			}
		}
		max = -999999;
		for (int i = 0; i < pla.length; i++) {
			if (max < pla[i].getGeld() + pla[i].getGesammtscore() && (i != p1 && i != p2 && i != p3)) {
				max = pla[i].getGeld() + pla[i].getGesammtscore();
				p4 = i;
			}
		}

		int[][] gesGeld = { { p1+1, pla[p1].getGeld() + pla[p1].getGesammtscore() },
				{ p2+1, pla[p2].getGeld() + pla[p2].getGesammtscore() },
				{ p3+1, pla[p3].getGeld() + pla[p3].getGesammtscore() },
				{ p4+1, pla[p4].getGeld() + pla[p4].getGesammtscore() } };
		Main.ende(gesGeld);
	}
	...
	public void werkMiete() {
		...
		int wurf = w1 + w2;
		Werk w = felder[geradeAmZug.getPos()].toWerk();
		spielfeld.zahleMiete(w.getGehoert(),
				(int) (w.getMiete() * (0.5 + (pla[w.getGehoert()].getWerkZahl() / 2))) * wurf);

		pla[w.getGehoert()].zahlen((int) (w.getMiete() * (0.5 + (pla[w.getGehoert()].getWerkZahl() / 2))) * wurf,
				geradeAmZugIndex);
		geradeAmZug.zahlen(-1 * (int) (w.getMiete() * (0.5 + (pla[w.getGehoert()].getWerkZahl() / 2))) * wurf,
				w.getGehoert());
	}
	...
	public boolean hausBauen(int pos) {
		...
		if (s.hausbauCheck(felder) && geradeAmZug.getGeld() - s.getHauskosten() * 1000 > -1
				&& geradeAmZug.getBesitz().contains(pos) && hausBauen) {
			s.hausBauen();
	...
	public void hausVerkaufen(int pos) {
		...
		Strasse s = felder[pos].toStrasse();
		if (s.hausVerkaufCheck(felder) && s.getHauszahl() > 0 && geradeAmZug.getBesitz().contains(pos) && hausBauen) {
			s.hausVerkaufen();
	...
	public void resumeECard() {
		switch (cardId) {
		case 0: // "Gehe in das Gefängnis. Begib dich dorthin. Gehe nicht über Los./nZiehe nicht
				// DM 4000 ein. "
			insGef();
			break;
		case 1: // "Du kommst aus dem Gefängnis./nDiese Karte muss behalten werden, bis sie
		...
		case 3: // "Gehe 3 Felder zurück"
			...
			positionsNotify();
			break;
		case 4: // "Rücke vor bist zum nächsten Bahnhof. Der Eigentümer erhält das doppelte der
				// normalen Miete./nHat noch kein Spieler einen Bahnhof, so kann er ihn von der
				// Bank kaufen."
			...
			positionsNotify();
			break;
		case 5: // "Mache einen Ausflug nach dem Süd-Bahnhof,/nund wenn Du über Los kommst,
				// ziehe DM 4000 ein."
		...
		case 9: // "Rücke vor bis zur Schlossallee."
			...
			positionsNotify();
			break;
		case 10: // "Lasse alle Deine Häuser renovieren./nZahle an die Bank/nfür jedes Haus DM
					// 500/nfür jedes Hotel DM2000"
			geradeAmZug.zahlen(-1 * ((geradeAmZug.getHausCounter() * 500) + (geradeAmZug.getHotelCounter() * 2000)),
					-1);
			break;
	...
	public void resumeGCard() {
		...
		case 2: // "Artzt-Kosten Zahle DM 1000."
			geradeAmZug.zahlen(-1000, -1);
			break;
		case 3: // "Du wirst zu Strassenausbesserungsarbeiten herangezogen. Zahle für Deine
				// Häuser und Hotels/nDM 800 je Haus/nDM 2300 je Hotel/nan die Bank."
			geradeAmZug.zahlen(-1 * ((geradeAmZug.getHausCounter() * 800) + (geradeAmZug.getHotelCounter() * 2300)),
					-1);
			break;
		case 4: // "Gehe in das Gefängnis. Begib Dich direkt dorthin. Gehe nicht über
				// Los./nZiehe nicht DM 4000 ein."
			insGef();
			break;
		case 5: // "Du kommst aus dem Gefängnis frei./nDiese Karte muss behalten werden, bis sie
				// gebraucht oder verkauft wird."
			geradeAmZug.addToGefFreiKarte(1);
			// Notfiy UI
			break;
		...
		case 10: // "Aus Lagerverkäufen erhältst Du/nDM 500."
			...
			break;
		case 11: // "Du hast den 2. Preis in einer Schönheitskonkurrenz gewonnen./nZiehe DM 200
					// ein."
			geradeAmZug.zahlen(200, -1);
			break;
